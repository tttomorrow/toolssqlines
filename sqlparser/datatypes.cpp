...
bool SqlParser::ParseTypedVariable(Token *var, Token *ref_type)
{
    ...
    if(cent != NULL)
    {
        ...
        if(type != NULL)
        {
            ...
            STATS_SET_CONV_NO_NEED(Target(SQL_ORACLE, SQL_MARIADB_ORA, SQL_POSTGRESQL, SQL_NETEZZA, SQL_OPENGAUSS))
        ...
        else
        {
            ...
            STATS_SET_CONV_NO_NEED(Target(SQL_ORACLE, SQL_MARIADB_ORA, SQL_POSTGRESQL, SQL_OPENGAUSS, SQL_NETEZZA))
    ...
    if(Source(SQL_ORACLE, SQL_MARIADB_ORA, SQL_POSTGRESQL, SQL_OPENGAUSS, SQL_NETEZZA) && 
        Target(SQL_ORACLE, SQL_MARIADB_ORA, SQL_POSTGRESQL, SQL_OPENGAUSS, SQL_NETEZZA))
        return true;
    else
    // Convert %TYPE
    if(type != NULL)
...
bool SqlParser::ParseBigdatetimeType(Token *name)
{
    ...
    if(_target == SQL_SQL_SERVER)
        ...
        if(Target(SQL_ORACLE, SQL_OPENGAUSS, SQL_POSTGRESQL, SQL_OPENGAUSS) == true)
...
bool SqlParser::ParseBitType(Token *name)
{
    ...
    if(open != NULL && Target(SQL_MARIADB, SQL_MYSQL, SQL_POSTGRESQL, SQL_OPENGAUSS, SQL_SYBASE_ASA) == false)
    {
        int size;
        int rem;
    ...
    if(varying == NULL)
    {
        ...
        if(_target == SQL_ORACLE)
        {
            ...
            if(Source(SQL_MYSQL, SQL_POSTGRESQL, SQL_OPENGAUSS) == true)
            {
                Token::Change(name, "RAW", L"RAW", 3);
...
bool SqlParser::ParseBlobType(Token *name)
{
    ...
    if(Target(SQL_POSTGRESQL, SQL_OPENGAUSS, SQL_GREENPLUM) == true)
        Token::Change(name, "BYTEA", L"BYTEA", 5);
                ...
                if(varying != NULL)
                {
                    ...
                    if(_target == SQL_ORACLE && _source != SQL_ORACLE)
                    {
                        ...
                        if((_target == SQL_POSTGRESQL && open == NULL && _source != SQL_POSTGRESQL) || 
                           (_target == SQL_OPENGAUSS && open == NULL && _source != SQL_OPENGAUSS))
                            Append(varying, "(1)", L"(1)", 3);
                        else
                            // Size is mandatory in MySQL
...
bool SqlParser::ParseDatetime2Type(Token *name)
{
    ...
    if(Target(SQL_POSTGRESQL, SQL_REDSHIFT, SQL_OPENGAUSS))
    {
        Token::Change(name, "TIMESTAMP", L"TIMESTAMP", 9);
...
bool SqlParser::ParseDateType(Token *name)
{
    ...
    if(_source == SQL_ORACLE)
    {
        ...
        if(Target(SQL_DB2, SQL_POSTGRESQL, SQL_GREENPLUM, SQL_ESGYNDB, SQL_OPENGAUSS))
            Token::Change(name, "TIMESTAMP(0)", L"TIMESTAMP(0)", 12);
    }
    else
    // Remove [] for other databases
...
bool SqlParser::ParseFloatType(Token *name)
{
    ...
    if((_target == SQL_POSTGRESQL && _source != SQL_POSTGRESQL) ||
       (_target == SQL_OPENGAUSS && _source != SQL_OPENGAUSS))
    {
        Token::Change(name, "DOUBLE PRECISION", L"DOUBLE PRECISION", 16);
        conv = true;
    ...
    else
    {
        ...
        if(_target != SQL_ORACLE)
        {
            ...
            if(prec != -1 && prec < 3 && sc == 0)
            {
                ...
                if(Target(SQL_SQL_SERVER, SQL_POSTGRESQL, SQL_GREENPLUM, SQL_OPENGAUSS))
                    Token::Change(name, "SMALLINT", L"SMALLINT", 8);
                else
                    Token::Change(name, "TINYINT", L"TINYINT", 7);
...
bool SqlParser::ParseTimeType(Token *name)
{
    ...
    if(without != NULL || with != NULL)
    {
        ...
        if(_target != SQL_POSTGRESQL && _target != SQL_OPENGAUSS && without != NULL)
...
bool SqlParser::ParseUniqueidentifierType(Token *name)
{
    ...
    if(Target(SQL_ORACLE, SQL_MYSQL, SQL_POSTGRESQL, SQL_OPENGAUSS) == true)
...
bool SqlParser::ParseVarbitType(Token *name)
{
    ...
    // Calculating RAW and BINARY size in bytes
    if(open != NULL && Target(SQL_POSTGRESQL, SQL_SYBASE_ASA, SQL_OPENGAUSS) == false)
    {
        int size;
        int rem;
...
bool SqlParser::ParseXmltypeType(Token *name)
{
    ...
    if(Target(SQL_SQL_SERVER, SQL_DB2, SQL_POSTGRESQL, SQL_SYBASE_ASA, SQL_OPENGAUSS) == true)
